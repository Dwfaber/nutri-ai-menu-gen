...
BEGIN
    ...
    SELECT COUNT(*) - COUNT(DISTINCT COALESCE(solicitacao_id, 0), COALESCE(produto_base_id, 0))
    INTO duplicates_count
    FROM co_solicitacao_produto_listagem
    WHERE produto_base_id IS NOT NULL;
    
    RAISE NOTICE 'Duplicatas encontradas: %', duplicates_count;
    ...
    INSERT INTO sync_logs (
        ...
        cleaned_count,
        jsonb_build_object(
            'duplicates_before', duplicates_count,
            'records_removed', cleaned_count,
            'cleanup_strategy', 'keep_most_recent',
            'cleanup_date', now()
        )
    );
    
    -- Verificar contagem final
    SELECT COUNT(*) INTO cleaned_count FROM co_solicitacao_produto_listagem;
    RAISE NOTICE 'Registros restantes após limpeza: %', cleaned_count;
